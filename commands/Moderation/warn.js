...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('warn')
        .setDescription(
            "[STAFF] Permet aux modérateur de donner un avertissement à un membre."
        )
        ...
        try {
            try {
                ...
                await user.send({
                    embeds: [{
                        color: red,
                        title: `⚠️ Vous avez été avertis sur le serveur ${interaction.guild.name}`,
                        description: `Vous avez été avertis par ${author}.`,
                        fields: [
            ...
            await db.execute(
                ...
                [user.id, author.id, reason, now, "warn", false]
            );

            await interaction.reply({
                embeds: [{
                    color: red,
                    title: `⚠️ Un utilisateur a été avertis !`,
                    url: "https://lunik-commu.whst.fr/appeals",
                    description: `> ${user} à été avertis par ${author}`,
                    thumbnail: {
            ...
            console.error("\x1b[31m ⟭ An error has occurred when trying to warn the user : ", error);
            
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    content: "Bon, il y a une erreur qui est survenue lors de l'avertissement du membre...",
                    flags: MessageFlags.Ephemeral
                });
            } else {
