...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('ban')
        .setDescription(
            "[STAFF] Permet aux modérateur de bannir un membre du serveur."
        )
        ...
        try {
            try {
                ...
                await user.send({
                    embeds: [{
                        color: red,
                        title: `🚨 Vous avez été banni du serveur ${interaction.guild.name}`,
                        description: `Vous avez été banni du serveur par ${author}.`,
                        fields: [
            ...
            await db.execute(
                ...
                [user.id, author.id, reason, now, "ban", false]
            );

            await interaction.reply({
                embeds: [{
                    color: red,
                    title: `👮‍♂️ Un utilisateur a été bannis !`,
                    url: "https://lunik-commu.whst.fr/appeals",
                    description: `> ${user} à été bannis par ${author}`,
                    thumbnail: {
            ...
            console.error("\x1b[31m ⟭ An error has occurred when trying to ban the user : ", error);
            
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    content: "Bon, il y a une erreur qui est survenue lors du bannissement du membre...",
                    flags: MessageFlags.Ephemeral
                });
            } else {
